...
Language-only postures (Ruby / RSpec / gem tooling). Architecture craft lives on `architecture` branch refs — never duplicate it here as a parallel doctrine log. Test-quality review judgment lives in `code-review` `tests` — this file holds Ruby/RSpec execution postures for `$dev` implement.
