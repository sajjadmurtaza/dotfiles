...
End-to-end PR review + publish (retrieve → review → reconcile drafts → submit) is owned by the **`code-review` skill `publish`** execution and its `reference/github-state.md`. This file remains the API helper for posting already-verified findings and for resolve/reply.
