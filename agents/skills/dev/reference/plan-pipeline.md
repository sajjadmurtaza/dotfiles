...
8. Pre-ship checklists embed ([`code-review/reference/plan-checklists.md`](../../code-review/reference/plan-checklists.md) — not full findings)
9. Plan ready — no code until user approves execute

Execute approved → `$dev` `implement` → validate → commit per phase → post-delivery **`code-review` findings** → if user asked to land, **`pull-request` `open`**.
