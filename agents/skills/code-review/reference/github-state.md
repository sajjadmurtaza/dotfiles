...
Pending-review and thread mutations for the `code-review` skill **`publish`** execution. Ordinary review publishing uses `event: COMMENT` only. Never submit `REQUEST_CHANGES` or `APPROVE` from this skill.
